...
def rotate_inplace_v3(values2dim):
    _, max_x = get_dimension(values2dim)
    _rotate_inplace_v3_helper(values2dim, 0, max_x - 1)


def _rotate_inplace_v3_helper(values2dim, left, right):
    if left >= right:
        return
    ...
    _rotate_inplace_v3_helper(values2dim, left + 1, right - 1)
