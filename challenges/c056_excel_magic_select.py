...
def generate_following_values_v2(current_value, sequence_length):
    return [value for value in range(current_value, current_value + sequence_length)]


def generate_following_values_v3(current_value, sequence_length):
...
def generate_following_values_for_predefined(predefined_values, current_value, sequence_length):
    ...
    while sequence_length > 0:
        result.append(current_value)
        current_value, current_pos = next_cyclic(
            predefined_values, current_pos)
        sequence_length -= 1
    return result
...
def main():
    ...
    print(generate_following_values_v3(1, 7))
    print(generate_following_values_v3(3, 11))
